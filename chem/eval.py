...
def calculate_molecule_properties(smiles_str):
    try:
        ...
        return True, qed_value, logp_value, mol_wt, mol_len
...
def get_raw_data_stats(lm_output, tokenizer, verbose=False):
    ...
    for decoded in tqdm(lm_output):
        ...
        valid, qed, logp, mol_wt, mol_len = calculate_molecule_properties(
            smiles_str)
        if valid:
            num_valid += 1
        ...
        if verbose:
            print(
                f"Molecule: {smiles_str}, valid: {valid}, QED: {qed}, LogP: {logp}, MolWt: {mol_wt}, MolLen: {mol_len}")
